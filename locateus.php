...
<body style="background-image:url(images/2.jpg)">
	...
	<div class="sucontainer" style="background-image:url(images/3.jpg)">
		<ul style="background-image:url(images/3.jpg)">			
			<label><b>ค้นหาหมอ</b></label>
			<input type="text" name="doctorname" placeholder="ใส่ชื่อหมอ หรือ แผนกที่ต้องการักษาเช่น ทันตกรรม"></input>
			<button type="submit" style="position:center" name="subd" value="Submit">ยืนยัน</button>
		</ul>
...
if(isset($_POST['subd']))
{
		...
		if(!empty($_POST['doctorname']))
		{
			...
			{
			echo "<hr>";
			echo "<label><b><br>".$row1['name']."<br><b>เพศ:<b>".$row1['gender']."<br><b>ความดชี่ยวชาญ:<b>".$row1['specialization']."<br></b>"."</label>";
			$sql2="select * from doctor_availability where did=".$row1["did"];
			$result2=mysqli_query($conn, $sql2);  
			while($row2 = mysqli_fetch_array($result2))
			{
			echo "<label><br><b>วัน:".$row2["day"]."</b><br><b>ตั้งแต่เวลา:<b>".$row2["starttime"]." ถึง ".$row2["endtime"]."</label>";
			$sql3="Select * from clinic where cid = ".$row2["cid"];
			$result3 = mysqli_query($conn , $sql3);
			while($row3 = mysqli_fetch_array($result3))
			{
				echo"<label><br><b>ชื่อคลินิก:".$row3["name"]." จังหวัด:".$row3["town"]." เมือง:".$row3["city"]."</label>";
			}
...
if(isset($_POST['submit']))
{
		...
		echo "<label><b>ชื่อคลินิก:".$row1['name']."</b><br><b>ที่อยู่:<b>".$row1['address']."<br><b>การติดต่อ:<b>".$row1['contact']."<br><b>ชื่อหมอ:<b>".$row2['name']."<br><b>ความชำนาญ:<b>".$row2['specialization']."<br><b>การติดต่อหมอ:<b>".$row2['contact']."</label>"; 
}
?>
	</form>
</body>
